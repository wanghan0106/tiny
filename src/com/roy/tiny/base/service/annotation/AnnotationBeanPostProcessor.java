package com.roy.tiny.base.service.annotation;
...
public class AnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	public AnnotationBeanPostProcessor() {
		...
		this.autowiredAnnotationTypes.add(Dao.class);
		this.autowiredAnnotationTypes.add(Value.class);
		ClassLoader cl = AnnotationBeanPostProcessor.class.getClassLoader();
		try {
	...
	public void setBeanFactory(BeanFactory beanFactory) throws BeansException {
		...
		daoFactory = (DaoFactory) this.beanFactory.getBean(DAO_FACTORY);
	}
		...
		do {
			...
			for (Field field : targetClass.getDeclaredFields()) {
				...
				if (annotation != null) {
					...
					if(annotation instanceof Dao) {
						currElements.add(new DaoFieldElement(field));
					} else {
						boolean required = determineRequiredStatus(annotation);
			...
			for (Method method : targetClass.getDeclaredMethods()) {
				...
				if (annotation != null && method.equals(ClassUtils.getMostSpecificMethod(method, clazz))) {
					...
					if(annotation instanceof Dao) {
						currElements.add(new DaoMethodElement(method, pd));
					} else {
						boolean required = determineRequiredStatus(annotation);
	...
	private void processCycleInjection(Object object) throws Throwable {
		if(object == null) {
			return;
		}
			...
			try {
				...
				if (this.cached) {
					...
					for (int i = 0; i < arguments.length; i++) {
						...
						processCycleInjection(arguments[i]);
	 ...
	 * Class representing injection information about an annotated field.
	 */
	private class DaoFieldElement extends InjectionMetadata.InjectedElement {

		public DaoFieldElement(Field field) {
			super(field, null);
		}
		...
		protected void inject(Object bean) throws Throwable {
			Field field = (Field) this.member;
			try {
				ReflectionUtils.makeAccessible(field);
				Object value = null;
				try {
					...
					logger.debug("Could not get bean:"+field.getName());
				}
				if(value==null) {
					ParameterizedType ptype = (ParameterizedType) field.getGenericType();
					Class type = (Class) ptype.getActualTypeArguments()[0];
					value = daoFactory.getDao(type);
					processCycleInjection(value);
				}
				field.set(bean, value);
			}
	 ...
	 * Class representing injection information about an annotated method.
	 */
	private class DaoMethodElement extends InjectionMetadata.InjectedElement {

		public DaoMethodElement(Method method, PropertyDescriptor pd) {
			super(method, pd);
		}
		...
		protected void inject(Object bean) throws Throwable {
			...
			try {
				ReflectionUtils.makeAccessible(method);
				String fieldName = this.pd.getName();
				Object value = null;
				try {
					value = beanFactory.getBean(fieldName);
				} catch(Throwable ex) {
					logger.debug("Could not get bean:"+fieldName);
				}
				if(value == null) {
